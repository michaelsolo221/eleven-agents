...
3. **Email Provider**: **Resend** API via the official `resend` Python SDK. Initial setup will use `onboarding@resend.dev` to allow rapid testing without immediate DNS configuration, targeting `michael@michael-lo.com`. **Update 2026-07-29 (issue #55)**: production has cut over to the verified custom domain sender (`lodgement@claims.michael-lo.com`, per ADR/issue #53) and the real `NOTIFICATION_EMAIL` recipient; the sandbox `onboarding@resend.dev` address is no longer used anywhere.
